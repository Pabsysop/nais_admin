...
use ic_agent::{
    ...
    agent::{
        ...
        signed::{SignedUpdate, SignedQuery, SignedRequestStatus},
        Replied
    },
    export::Principal,
};
...
pub fn exec_cmd(cmd: &SubCommand){
    match cmd {
...
/// Create a life
pub async fn create_life() -> Result<Json<UserResponse>, ApiError> {
    validate(&params)?;
...
/// Create a board
pub async fn create_board() -> Result<Json<UserResponse>, ApiError> {
    validate(&params)?;
